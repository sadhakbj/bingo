...
class ServeCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($this->app->router->getRoutes() as $name => $route) {
            ...
            $path    = $route->getPath();
            $this->log($output, 'LOG', 'RouterExplorer', "Mapped {<fg=cyan>{$methods}</> <fg=white>{$path}</>} route");
        }
        ...
        // stdout goes to the same pipe as stderr so VarDumper CliDumper (php://stdout)
        // is still visible here; Symfony HTTP responses do not rely on that FD.
        $process = proc_open(
            PHP_BINARY . ' -S ' . escapeshellarg("{$host}:{$port}") . ' -t ' . escapeshellarg($public),
            [0 => STDIN, 1 => ['file', 'php://stderr', 'w'], 2 => ['pipe', 'w']],
            $pipes,
        );
        ...
        while (!feof($pipes[2])) {
            ...
            if (preg_match('/\[.+?\] [\d.]+:\d+ \[(\d+)\]: (\w+) (.+)/', $line, $m)) {
                ...
                $this->log(
                    ...
                    $level,
                    'HTTP',
                    sprintf('<%s>%-7s</>  %s  → <%s>%s</>', $methodColor, $method, $path, $statusColor, $status),
                );
