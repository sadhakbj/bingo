...
class RouterTest extends TestCase
{
    ...
    public function test_dispatch_prefix_with_index_route_accepts_trailing_slash(): void
    {
        $this->router->registerController(StubApiController::class);

        $withSlash    = $this->router->dispatch($this->makeRequest('/stub/', 'GET'));
        $withoutSlash = $this->router->dispatch($this->makeRequest('/stub', 'GET'));

        $this->assertSame(200, $withSlash->getStatusCode());
    ...
    public function test_dispatch_unknown_route_on_plain_controller_throws_not_found(): void
    {
        $this->router->registerController(StubPlainController::class);
        $request = $this->makeRequest('/plain/does-not-exist', 'GET');
