...
> A PHP 8.5+ framework for API-first development — **automatic controller discovery**, attribute-driven routing, typed configuration, dependency injection, and Laravel Eloquent ORM, with zero boilerplate.

Built from scratch on top of Symfony's HTTP, Routing, Validator, Console, and DI components. Just add `#[ApiController]` to your class — routes are discovered automatically, no manual registration needed.
...
**Quick Start — Create Your First Route:**
...
That's it! No registration needed. Hit `http://127.0.0.1:8000/api/hello` — your route works immediately.
...
├── storage/
│   ├── framework/
│   │   └── discovery.php     # Auto-generated discovery cache (git-ignored)
│   ├── logs/                 # Application logs
│   └── rate-limit/           # File-based rate limit counters (dev fallback)
├── tests/
└── .env
```
...
**Note:** The `storage/framework/discovery.php` file is auto-generated by the [Auto-Discovery](#auto-discovery) system. It's git-ignored and rebuilds automatically in development.
...
```
public/index.php
  → bootstrap/app.php          (DI bindings, config)
  → Application::run()
          ...
          → SecurityHeadersMiddleware
            → RequestIdMiddleware
              → RateLimitMiddleware (production only)
                → Router::dispatch()
                    Reflection resolves controller + method params
...
Bingo automatically discovers controllers, commands, and middleware via PHP attributes — **no manual registration required**. Just create a controller with `#[ApiController]` or route attributes (`#[Get]`, `#[Post]`, etc.), and it's instantly available.

### How It Works

The discovery system scans `app/Http/Controllers/` and `app/Console/Commands/` at startup, extracts metadata from PHP attributes using Reflection, and caches the results in `storage/framework/discovery.php`.

**What gets discovered:**
- Controllers with `#[ApiController]`, `#[Get]`, `#[Post]`, `#[Put]`, `#[Patch]`, `#[Delete]` attributes
- Route metadata: paths, HTTP methods, middleware, throttle limits
- Console commands extending `Symfony\Component\Console\Command\Command`

**Example — no registration needed:**
...
#[Middleware([AuthMiddleware::class])]  // Applied to all routes
class PostsController
{
    #[Get('/')]
    public function index(): Response
    {
        // Automatically discovered and registered at GET /api/posts
    }

    #[Post('/')]
    public function create(#[Body] CreatePostDTO $dto): Response
    {
        // Automatically discovered and registered at POST /api/posts
    }
}
```

That's it! No entries in `bootstrap/app.php`, no manual route registration. The framework finds it automatically.

### Development vs Production

**Development Mode** (`APP_ENV=development`):
- **First request without cache:** Auto-generates `storage/framework/discovery.php` (~50ms one-time cost)
- **Subsequent requests:** Checks filemtime; uses cache if files unchanged (~2ms)
- **After file changes:** Auto-rebuilds cache on next request
- **Zero configuration needed** — just code and refresh

**Production Mode** (`APP_ENV=production`):
- **Cache MUST exist before first request** — app throws `RuntimeException` if missing
- **Never auto-generates** — ensures predictable performance and no surprise delays
- Run `php bin/bingo discovery:generate` during deployment (part of build step)
- **Zero runtime scanning** — routes load from pre-built cache in ~2ms

### Cache Commands

**Generate cache (for production deployments):**
```bash
...
Output:
```
✓ Discovery cache generated successfully!
...
Cache location: storage/framework/discovery.php
```

**Clear cache (forces rebuild on next request):**
```bash
...
```bash
composer install
php bin/bingo discovery:generate  # Pre-build cache
# Deploy
```
...
The discovery cache is smart:

- **Development**: Checks file modification times (filemtime) — rebuilds only when files actually change
- **Production**: Never scans at runtime — cache is immutable until next deployment
- **Cache location**: `storage/framework/discovery.php` (git-ignored)

You can delete the cache anytime in development — it regenerates automatically on the next request. In production, deleting the cache will cause the application to fail-fast with a clear error message.

---

## Routing

Routes are declared directly on controller methods using PHP attributes. **No route files, no manual registration** — controllers are auto-discovered via the [Auto-Discovery](#auto-discovery) system.
...
That's it! The controller is automatically discovered and all routes are registered. No entries needed in `bootstrap/app.php`.

> **Route order:** Declare specific routes (`/search`, `/upload`) before wildcard routes (`/{id}`) within the same controller class, or the wildcard will match first.
...
```bash
php bin/bingo discovery:generate    # alias: discovery:cache
```

Scans `app/` and writes a cache file to `storage/framework/discovery.php` with all discovered controllers, routes, commands, and metadata. Run this in your CI/CD pipeline before deploying.
...
Deletes the discovery cache. In development, the cache rebuilds automatically on the next request. In production, this will cause the application to fail until you regenerate the cache.
...
Displays a table of all registered routes with their HTTP methods, paths, controllers, actions, and middleware.
